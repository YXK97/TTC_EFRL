...
from ..utils.scaling import scaling_calc, scaling_calc_bound, calc_rsh_distance_bound, calc_rsh_distance


INF = jnp.inf


class MVELaneChangeAndOverTake_RSH(MVE):
    ...
    PARAMS = {
        ...
        "rsh_thresh": 0.2 #rsh直接用距离表示安全，安全裕度为0.2
    }
    PARAMS.update({
    ...
    def __init__(self,
        ...
        area_size = MVELaneChangeAndOverTake_RSH.PARAMS["rollout_state_range"][:4] if area_size is None else area_size
        params = MVELaneChangeAndOverTake_RSH.PARAMS if params is None else params
        super(MVELaneChangeAndOverTake_RSH, self).__init__(num_agents, area_size, max_step, max_travel, dt, reward_min, reward_max, params)
    ...
    def get_cost(self, graph: MVEEnvGraphsTuple) -> Tuple[Cost, Cost]:
        ...
        if num_agents == 1:
            ...
            h_pairs = jax.vmap(calc_rsh_distance, in_axes=(0, 0))(
                state_i_pairs,
                state_j_pairs
            )

            h_matrix = jnp.full((num_agents, num_agents), INF)
            h_matrix = h_matrix.at[i_pairs, j_pairs].set(h_pairs)

            a_agent_cost = jnp.max(thresh - h_matrix, axis=1)
            a_agent_cost_real = jnp.max(0.0 - h_matrix, axis=1)

        # ======================================================
        # agent-obstacle RSH cost
        # ======================================================
        if num_obsts == 0:
            ...
            h_matrix = h_pairs.reshape((num_agents, num_obsts))

            a_obst_cost = jnp.max(thresh - h_matrix, axis=1)
            a_obst_cost_real = jnp.max(0.0 - h_matrix, axis=1)
        ...
        # lower y bound: y >= yl
        n_low = jnp.array([0.0, 1.0])
        b_low = jnp.array([yl])

        h_bound_yl = jax.vmap(calc_rsh_distance_bound, in_axes=(0, None, None))(
            agent_states,
            n_low,
            b_low
        )

        a_bound_yl_cost = thresh - h_bound_yl
        a_bound_yl_cost_real = 0.0 - h_bound_yl

        # upper y bound: y <= yh
        # 等价于 -y >= -yh
        n_high = jnp.array([0.0, -1.0])
        b_high = jnp.array([-yh])

        h_bound_yh = jax.vmap(calc_rsh_distance_bound, in_axes=(0, None, None))(
            agent_states,
            n_high,
            b_high
        )

        a_bound_yh_cost = thresh - h_bound_yh
        a_bound_yh_cost_real = 0.0 - h_bound_yh

        cost = jnp.stack(
            [a_agent_cost, a_obst_cost, a_bound_yl_cost, a_bound_yh_cost],
            axis=1
        )

        cost_real = jnp.stack(
            [a_agent_cost_real, a_obst_cost_real, a_bound_yl_cost_real, a_bound_yh_cost_real],
            axis=1
        )
