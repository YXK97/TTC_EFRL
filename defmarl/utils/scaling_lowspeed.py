...
@jax.jit
def rear_to_center(s: State) -> Array:
    """
    ...
    x_center = x_rear + lr * jnp.cos(theta)
    y_center = y_rear + lr * jnp.sin(theta)
...
def scaling_calc(s1: State, s2: State) -> Array:
    ...
    # stste: x y θ v δ bw bh lr
    O1 = rear_to_center(s1)
    O2 = rear_to_center(s2)
    # 计算 host 和 agent/obst 的顶点， host/agent/obst均为矩形
    Q1 = calc_2d_rot_matrix(s1[2])
    m_V = jnp.array([[s1[5] / 2, s1[6] / 2],
                     [s1[5] / 2, -s1[6] / 2],
                     [-s1[5] / 2, s1[6] / 2],
                     [-s1[5] / 2, -s1[6] / 2]])
    m_V = O1 + m_V @ Q1.T
    Q2 = calc_2d_rot_matrix(s2[2])
    n_P = jnp.array([[s2[5] / 2, s2[6] / 2],
                     [s2[5] / 2, -s2[6] / 2],
                     [-s2[5] / 2, s2[6] / 2],
                     [-s2[5] / 2, -s2[6] / 2]])
    n_P = O2 + n_P @ Q2.T
    ...
    A1 = Ao @ Q1.T
    A2 = Ao @ Q2.T
    b1o = jnp.array([s1[5] / 2, s1[5] / 2, s1[6] / 2, s1[6] / 2])
    b2o = jnp.array([s2[5] / 2, s2[5] / 2, s2[6] / 2, s2[6] / 2])
    b1 = b1o + A1 @ O1
    b2 = b2o + A2 @ O2
...
def scaling_calc_bound(s: State, A: Array, b: Array) -> Array:
    ...
    O = rear_to_center(s)
    # 计算host的顶点，host为矩形
    Q = calc_2d_rot_matrix(s[2])
    m_V = jnp.array([[ s[5]/2,  s[6]/2],
                     [ s[5]/2, -s[6]/2],
                     [-s[5]/2,  s[6]/2],
                     [-s[5]/2, -s[6]/2]])
    m_V = O + m_V @ Q.T
